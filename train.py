...
def load_network(network, base_model_name):
    save_path = os.path.join('./model', base_model_name, 'net_last.pth')
...
if opt.PCB and opt.LSTM:
    base_model_name = 'PCB_Effi_NLAC_RE_WE'
    model = load_network(model, base_model_name)
...
if not opt.PCB or opt.LSTM:
    ...
    optimizer_ft = torch.optim.Adam([{'params': base_params, 'lr': 0.1*opt.lr},
    ...
    ignored_params += (list(map(id, model.classifierA0.parameters()))
